...
#[inline]
pub fn hamming_bitwise_fast_64(x: &[u8], y: &[u8]) -> u64 {
    assert_eq!(x.len(), y.len());
...
#[doc(hidden)]
#[inline]
pub fn naive_hamming_distance_64(x: &[u8], y: &[u8]) -> u64 {
    assert_eq!(x.len(), y.len());
...
#[doc(hidden)]
#[inline]
pub fn naive_hamming_distance_iter_64(x: &[u8], y: &[u8]) -> u64 {
    x.iter()
        .zip(y)
        .fold(0, |a, (b, c)| a + (*b ^ *c).count_ones() as u64)
}
